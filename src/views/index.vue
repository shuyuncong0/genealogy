    ...
    <el-container >
      <el-aside width="380px">
        <el-card class="box-card">
          ...
          <div  class="text item">
             <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 345px;">
              ...
              <el-form-item label="长次">
                <el-select v-model="temp.elderOrder" class="filter-item" placeholder="请选择长次">
                  <el-option v-for="item in elderOrderOptions" :key="item.key" :label="item.value" :value="item.key" />
                </el-select>
              </el-form-item>
...
export default {
  data() {
    return {
      ...
      elderOrderOptions: [{key:1,value:'长子'}, {key:2,value:'次子'},{key:3,value:'三子'},{key:4,value:'四子'},{key:5,value:'幼子'}],
  ...
  methods: {
    ...
    renderContent(h, data) {
      ...
      return (
          <div style="height: 100px;width:135px">
              <div style="">
                <span style="border-right:1px solid #ccc;padding: 7px;">{data.elderOrder}</span>
            ...
            <div  style="border:1px solid #ccc;margin-top: 7px;">
              <div style="float: left;padding: 7px 3px;border-right:1px solid #ccc;width: 78px;">
                <div style="border-bottom:1px solid #ccc;padding: 7px 3px;">{data.epithet}</div>
                <div style="padding: 7px 3px;">{data.username}</div>
              </div>
              <div style="float: right;width: 48px;">
                <span style="transform: translateY(120%);    float: left;margin: 9px 0px;">{data.wifeLastname}</span>
              </div>             
             </div>
          </div>
          );
    ...
    onNodeClick(e, data) {
      ...
      _this.temp.pusername = _this.dataList.find(item => item.userId === data.pid)?
    ...
    parseTree(list, callback, targetList) {
      ...
      if (targetList) {
        ...
        // 数据分批处理
        if ( _this.parseTree.stackIndexSize < ( _this.parseTree.maxStackSize || 50000)) {
          targetList.forEach(({children}) =>  _this.parseTree(list, callback, children));
        } else {
    ...
    getDicts(val){
      ...
      for(var item of this.elderOrderOptions) { // item代表数组里面的元素
          if(item.value == val){
            return item.key;
          }
      }
